...
export async function POST(req: NextRequest) {
  try {
    ...
    // Create a new job for every scan if no jobId was provided
    ...
    if (!activeJobId) {
      ...
      if (jobError) {
        return NextResponse.json({ error: 'Failed to create scan job: ' + jobError.message }, { status: 500 })
      }
    ...
    if (activeJobId) {
      await supabase.from('keen_web3_jobs').update({ status: 'running', started_at: new Date().toISOString() }).eq('id', activeJobId)
    }
    ...
    if (!aiResp.ok) {
      ...
      if (activeJobId) await supabase.from('keen_web3_jobs').update({ status: 'error', error: err }).eq('id', activeJobId)
    ...
    if (activeJobId && signals.length > 0) {
      ...
      await supabase.from('keen_web3_jobs').update({ status: 'done', completed_at: new Date().toISOString(), signals_count: 0 }).eq('id', activeJobId)
    }

    return NextResponse.json({ ok: true, signals, count: signals.length, contractName, jobId: activeJobId })
  } catch (e: any) {
